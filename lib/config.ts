...
const sessionSecret = secret("SESSION_SECRET", "change-me-in-production-serenity-mail");
...
export const env = {
  ...
  adminEmail: process.env.ADMIN_EMAIL ?? "admin@serenity.local",
  adminPassword: secret("ADMIN_PASSWORD", "serenity-local-2026"),
  sessionSecret,
  databaseUrl: secret("DATABASE_URL", "postgres://serenity:serenity@localhost:5432/serenity_mail"),
  ...
  s3Prefix: (process.env.S3_PREFIX ?? "serenity-mail").replace(/^\/+|\/+$/g, ""),
...
export function productionConfigurationChecks() {
  ...
  const checks = [
    ...
    { key: "admin_password", ok: !production || (env.adminPassword.length >= 16 && env.adminPassword !== "serenity-local-2026"), required: production, detail: "La contraseña de bootstrap no puede ser la local." },
    ...
    { key: "database_credentials", ok: !production || !env.databaseUrl.includes("serenity:serenity@"), required: production, detail: "PostgreSQL no puede conservar la contraseña local." },
