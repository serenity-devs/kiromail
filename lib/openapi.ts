...
import { subscriberTableColumnIds } from "./list-table-columns";
...
const subscriberTableColumnsSchema: Schema = { type: "array", maxItems: subscriberTableColumnIds.length, uniqueItems: true, items: { type: "string", enum: subscriberTableColumnIds }, description: "Campos globales visibles en la tabla de suscriptores, en el orden elegido." };
...
export const openApiDocument = {
  ...
  paths: {
    ...
    "/api/v1/campaigns/{id}/resend-non-openers": {
      get: operation("Estimar el reenvío a no abiertos", "Campañas", { scope:"campaigns:read",parameters:[campaignId],responseSchema:ref("NonOpenerResendPreview"),description:"Cuenta destinatarios enviados sin una apertura humana registrada y vuelve a aplicar bajas, bloqueos, supresiones y exclusiones." }),
      post: operation("Crear un borrador para no abiertos", "Campañas", { scope:"campaigns:write",status:"201",parameters:[campaignId],requestBody:jsonBody({type:"object",properties:{name:{type:"string",maxLength:200}}}),responseSchema:ref("Campaign"),description:"Copia la campaña completada como borrador con una audiencia dinámica ligada a sus destinatarios sin apertura. La audiencia se recalcula al lanzar." }),
    },
  ...
  components: {
    ...
    schemas: {
      ...
      VisualBlock: { type: "object", required: ["id", "type", "content"], properties: { id: { type: "string" }, type: { type: "string", enum: ["heading", "text", "button", "image", "divider", "spacer", "quote", "list"] }, content: { type: "string" }, url: { type: "string" }, link_url: { type: "string", description: "Enlace opcional al hacer clic en una imagen." }, asset_id: { type: "string", format: "uuid" }, alt: { type: "string" }, color: { type: "string" }, align: { type: "string", enum: ["left", "center", "right"] }, size: { type: "number" } } },
      ...
      Campaign: { type: "object", additionalProperties: true, properties: { id: { type: "string", format: "uuid" },version:{type:"integer"},duplicated_from_id:{type:["string","null"],format:"uuid"}, name: { type: "string" },target_type:{type:"string",enum:["all","list","tag","segment","non_openers"]},target_id:{type:["string","null"],format:"uuid"}, status: { type: "string",enum:["draft","pending_approval","scheduled","preparing","queued","sending","paused","completed","cancelled","failed"] },approval_required:{type:"boolean"},approved_at:{type:["string","null"],format:"date-time"},approved_version:{type:["integer","null"]}, total_recipients: { type: "integer" }, delivered_count: { type: "integer" } } },
      NonOpenerResendPreview:{type:"object",properties:{available:{type:"boolean"},eligible:{type:"integer",minimum:0},sent:{type:"integer",minimum:0},opened:{type:"integer",minimum:0},tracking_enabled:{type:"boolean"},reason:{type:["string","null"]}}},
      ...
      ApiKeyCreate: { type: "object", required: ["name", "scopes"], properties: { name: { type: "string" }, scopes: { type: "array", minItems: 1, items: { type: "string", enum: [...apiKeyScopes] } }, expires_at: { type: ["string", "null"], format: "date-time" } } },
