...
function codeFor(language: string, example: Example) {
  ...
  if (language === "JavaScript") return `const response = await fetch(\`${"${SERENITY_URL}"}${example.path}\`, {\n  method: "${example.method}",\n  headers: {\n    "Authorization": \`Bearer ${"${SERENITY_TOKEN}"}\`,\n    "Content-Type": "application/json",\n    "Idempotency-Key": crypto.randomUUID()\n  },\n  body: JSON.stringify(${body.split("\n").join("\n  ")})\n});\n\nconst result = await response.json();\nif (!response.ok) throw new Error(result.error?.message);\nconsole.log(result);`;
  if (language === "PHP") return `<?php\n$body = ${body.replace(/\{/g, "[").replace(/\}/g, "]").replace(/: /g, " => ").replace(/\n/g, "\n")};\n\n$ch = curl_init(getenv('SERENITY_URL') . '${example.path}');\ncurl_setopt_array($ch, [\n  CURLOPT_RETURNTRANSFER => true,\n  CURLOPT_POST => true,\n  CURLOPT_HTTPHEADER => [\n    'Authorization: Bearer ' . getenv('SERENITY_TOKEN'),\n    'Content-Type: application/json',\n    'Idempotency-Key: ' . bin2hex(random_bytes(16))\n  ],\n  CURLOPT_POSTFIELDS => json_encode($body)\n]);\n$result = curl_exec($ch);\nif (curl_getinfo($ch, CURLINFO_HTTP_CODE) >= 400) throw new RuntimeException($result);\necho $result;`;
  if (language === "Python") return `import os\nimport uuid\nimport requests\n\nresponse = requests.${example.method.toLowerCase()}(\n    os.environ["SERENITY_URL"] + "${example.path}",\n    headers={\n        "Authorization": f"Bearer {os.environ['SERENITY_TOKEN']}",\n        "Idempotency-Key": str(uuid.uuid4()),\n    },\n    json=${body.replace(/true/g, "True").replace(/false/g, "False").replace(/null/g, "None")}\n)\nresponse.raise_for_status()\nprint(response.json())`;
  return `curl --request ${example.method} "$SERENITY_URL${example.path}" \\\n  --header "Authorization: Bearer $SERENITY_TOKEN" \\\n  --header "Content-Type: application/json" \\\n  --header "Idempotency-Key: $(uuidgen)" \\\n  --data '${body}'`;
}
...
export function ApiDocs({ document }: { document: unknown }) {
  ...
  return <div className="api-docs-shell">
    <header className="api-docs-topbar">
      <div className="api-docs-brand"><span>S</span><strong>serenity mail</strong><em>API</em></div>
    ...
    <main className="api-docs-main">
      <section className="api-docs-hero">
        ...
        <aside><strong>Inicio rápido</strong><code>Authorization: Bearer serenity_••••</code><p>Crea la clave en Ajustes. El secreto completo solo aparece una vez.</p></aside>
      </section>
      <section className="api-example-panel">
        <div className="api-example-copy"><p className="eyebrow">Ejemplos ejecutables</p><h2>Empieza con una petición real</h2><p>Define <code>SERENITY_URL</code> y <code>SERENITY_TOKEN</code>. Las operaciones sensibles incluyen una clave idempotente.</p><label>Flujo<select value={exampleId} onChange={event=>setExampleId(event.target.value)}>{examples.map(item=><option key={item.id} value={item.id}>{item.label} · {item.scope}</option>)}</select></label></div>
