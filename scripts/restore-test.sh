...
test_database=${RESTORE_TEST_DATABASE:-serenity_restore_test}
if [ "$test_database" != "serenity_restore_test" ]; then echo "La prueba solo puede usar serenity_restore_test" >&2; exit 1; fi
...
psql "$database_url" -v ON_ERROR_STOP=1 -c "DROP DATABASE IF EXISTS serenity_restore_test WITH (FORCE)" >/dev/null
psql "$database_url" -v ON_ERROR_STOP=1 -c "CREATE DATABASE serenity_restore_test" >/dev/null
test_database_url=${RESTORE_TEST_DATABASE_URL:-${database_url%/*}/serenity_restore_test}
...
psql "$database_url" -v ON_ERROR_STOP=1 -c "DROP DATABASE serenity_restore_test WITH (FORCE)" >/dev/null
