...
test_db="kiromail_bootstrap_${suffix}"
if [[ ! "${test_db}" =~ ^kiromail_bootstrap_[0-9]{14}_[0-9]+$ ]]; then
...
database_url="postgres://kiromail:kiromail@postgres:5432/${test_db}"
...
counts="$("${postgres[@]}" psql -At -v ON_ERROR_STOP=1 -U kiromail -d "${test_db}" -c \
...
admin="$("${postgres[@]}" psql -At -v ON_ERROR_STOP=1 -U kiromail -d "${test_db}" -c \
