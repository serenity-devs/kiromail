...
try{
  ...
  const list=await api("/api/v1/lists",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:`ciclo_${runId.replaceAll("-","").slice(0,12)}`,name:`Ciclo campañas E2E ${runId.slice(0,8)}`,default_from_name:"Serenity E2E",default_from_email:"hola@serenity.local",default_reply_to:"hola@serenity.local",legal_footer:"Prueba controlada de ciclo de campañas",double_opt_in:false})});
