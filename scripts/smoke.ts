...
try {
  const login = await fetch(`${baseUrl}/api/auth/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: "admin@kiromail.local", password: "kiromail-local-2026" }) });
  ...
  await request("/api/test-email", { method: "POST", body: JSON.stringify({ template_id: initial.templates[0].id, email: "smoke-test@example.com", subject: "Plantilla de prueba", from_name: "KiroMail", from_email: "hola@kiromail.local", reply_to: "hola@kiromail.local" }) });
  ...
  const campaign = await request<{ id: string }>("/api/campaigns", { method: "POST", body: JSON.stringify({ name: "Prueba integral local", subject: "Correo local para {{first_name}}", preview_text: "Comprobación automática", from_name: "KiroMail", from_email: "hola@kiromail.local", reply_to: "hola@kiromail.local", template_id: initial.templates[0].id, target_type: "list", target_id: listId, scheduled_at: null }) });
