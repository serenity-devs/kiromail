...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (/^[0-9a-f-]{36}$/i.test(id)) await recordRecipientEvent(id, "open", { source: "pixel",user_agent:userAgent },undefined,isAutomatedInteraction(userAgent,purpose)).catch(() => undefined);
