...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (/^[0-9a-f-]{36}$/i.test(id)) await recordMessageEvent(id, "clicked", { source: "redirect", user_agent:userAgent }, destination.toString(),isAutomatedInteraction(userAgent,purpose)).catch(() => undefined);
