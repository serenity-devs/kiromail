...
export async function GET(request:Request){
  ...
  const lines=["# HELP serenity_up Application metrics endpoint is available","# TYPE serenity_up gauge",metric("serenity_up",1)];
  for(const[queue,counts]of Object.entries(dashboard.queues))for(const[state,value]of Object.entries(counts as Record<string,number>))lines.push(metric("serenity_queue_jobs",value,{queue,state}));
  for(const worker of dashboard.workers as unknown as {instance_id:string;healthy:boolean}[])lines.push(metric("serenity_worker_healthy",worker.healthy?1:0,{instance:worker.instance_id}));
  for(const state of states)lines.push(metric("serenity_messages",state.count,{kind:state.kind,status:state.status}));
  lines.push(metric("serenity_dead_letters_open",(dashboard.dead_letters as unknown as {status:string}[]).filter(item=>item.status==="open").length));
  lines.push(metric("serenity_http_errors_last_hour",Number(httpErrors.count)));
