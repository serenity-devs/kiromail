...
**Checkpoint actual:** `1.0.0-rc.2` está instalado en el VPS compartido con app, worker, PostgreSQL, Redis y backup aislados, sin puertos propios ni reinicios de la aplicación preexistente. GitHub publica imágenes inmutables verificadas y un timer del servidor las instala con backup, migración, healthcheck y rollback, sin intercambiar claves SSH ni tokens persistentes. DNS directo, HTTPS público, HSTS y renovación automática de Let's Encrypt están operativos. La revisión `cab4799b361825ce582c8fd46964b6989bad7a47` está desplegada y saludable. La suite 54/54, lint y build están verdes. La app está disponible para completar el onboarding; quedan la cuenta SES/SNS real, la identidad remitente, dirección postal y MFA antes de que readiness permita envíos.

| Bloque | Estado | Progreso verificable |
|---|---|---|
| 1. Fundamentos y datos | Completado | Migraciones `001`–`026` aplicadas desde cero y ensayadas desde la versión inmediatamente anterior. El esquema incluye recuperación MFA, políticas de retención, heartbeats, ejecuciones operativas, métricas agregadas, DLQ persistente, solicitudes de privacidad, trazas de fusión, procedencia de copias, blobs/tamaño MIME por mensaje, revisiones monotónicas de recursos editables y tema visual compartido por instalación. Índices parciales cubren comparativas por lista/segmento y señales de interacción. El backend S3, cifrado en servidor, gzip/hash y tareas de retención están implementados. |
...
- **2026-08-11 — Temas y repaso visual de Ajustes/Operaciones:** aplicada `025_ui_themes.sql` y desplegada la revisión `594367381c5fc9327854bdab03b6673aac660823`. Ajustes incorpora una sección completa de Apariencia con Kiro, Océano, Lavanda, Terracota y Grafito; cada tema cambia paleta, superficies, navegación y familias tipográficas, se previsualiza al instante, se persiste para toda la instalación y se restaura sin destello mediante almacenamiento local validado. La prueba en producción recorrió los cuatro temas alternativos, guardó Grafito, confirmó paleta y tipografía después de recargar y restauró Kiro con una segunda recarga. La misma prueba descubrió y corrigió que el `step` de tres umbrales porcentuales hacía inválidos los valores existentes y bloqueaba silenciosamente cualquier envío del formulario; los 54 tests incluyen la regresión. Los paneles de Operaciones tienen ahora 24 px de padding en escritorio y 18 px en móvil; navegador real midió 24 px computados y confirmó el resultado visual. GitHub Checks/Deploy, migración, contenedores y healthcheck terminaron correctamente.
- **2026-08-11 — Tema Terminal:** aplicada `026_terminal_theme.sql` y desplegada la revisión `cab4799b361825ce582c8fd46964b6989bad7a47`. El sexto tema añade negro profundo, paneles carbón, verde fósforo, ámbar y tipografía monoespaciada para títulos, controles y contenido; adapta también tablas, estados, formularios y superficies auxiliares para evitar zonas claras accidentales, conservando blancos únicamente en previsualizaciones de email. La selección se guardó en producción y una recarga completa confirmó `#070b0d`, `#40d98b` y la pila SFMono/Cascadia/Menlo. Terminal queda como tema activo de la instalación.
- **2026-08-11 — Fiabilidad de guardados:** desplegada la revisión `4eb664bb3332de0653509a2fcb2bb07ba1a6f666` tras auditar todos los formularios administrativos y corregir el bloqueo silencioso causado por validaciones de campos situados en otras pestañas. Ajustes cambia ahora a la pestaña problemática, enfoca el campo y muestra un mensaje concreto; el resto de formularios informa cuando la validación nativa impide enviar. Las acciones de envío declaran explícitamente `type="submit"` y el alta de listas/etiquetas recupera su estado y muestra el error si falla la API. En producción se reprodujo el caso de email inválido oculto al guardar Apariencia: el panel volvió a General con foco y aviso; crear una lista vacía mostró su error sin generar datos. Océano persistió tras recarga y Terminal se restauró como tema activo. La regresión automatizada exige cobertura de validación en todos los formularios; suite 57/57, lint, build, GitHub Checks/Deploy y healthcheck pasan.
- **2026-08-11 — Cobertura integral del tema Terminal:** desplegada la revisión `6a1647cdf3521b160c7bbba0d0f4bc15d511a6e1`. Se sustituyeron las superficies claras residuales de tablas, filtros, insignias, estados, formularios, modales, informes, entregabilidad, administración, referencia API y constructor de plantillas por la jerarquía semántica de panel, campo, fondo secundario y estado de Terminal; también se corrigieron textos oscuros fijados directamente en estados y etiquetas. Los blancos se conservan únicamente donde expresan contenido real: lienzos y previsualizaciones de email, además de las miniaturas que muestran cómo son los temas claros. El navegador confirmó ausencia de fondos claros accidentales en las nueve secciones principales, las ocho pestañas de Ajustes, el alta de suscriptor, `/api-docs` y `/plantillas/nueva`. Suite 58/58, lint, build, GitHub Checks/Deploy, actualización automática del VPS y healthcheck pasan; Terminal sigue activo en producción.
- **2026-08-11 — Gestión visual de claves API:** desplegada la revisión `1a3631a259b415d2e8f5630f8ab29aab8f7aebef`. Ajustes → API e integraciones incorpora listado de credenciales activas e históricas, creación con nombre, permisos agrupados y caducidad opcional, secreto visible una sola vez con copia, autor y fechas de creación/último uso/caducidad, y revocación inmediata confirmada. Solo los administradores acceden al gestor; cada creación y revocación conserva actor y detalle en auditoría. Formulario, endpoint y OpenAPI comparten una única fuente de scopes, la API rechaza caducidades pasadas y el listado nunca expone el hash ni el secreto. Suite 61/61, lint, build, GitHub Checks/Deploy y actualización automática del VPS pasan. La prueba funcional manual queda reservada al propietario y no se crearon claves temporales en producción.
