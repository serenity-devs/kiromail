...
- **2026-08-11 — Fiabilidad de guardados:** auditados todos los formularios administrativos y corregido el bloqueo silencioso causado por validaciones de campos situados en otras pestañas. Ajustes cambia ahora a la pestaña problemática, enfoca el campo y muestra un mensaje concreto; el resto de formularios informa cuando la validación nativa impide enviar. Las acciones de envío declaran explícitamente `type="submit"` y el alta de listas/etiquetas recupera su estado y muestra el error si falla la API. La regresión automatizada exige cobertura de validación en todos los formularios; suite 57/57, lint y build pasan.
