...
**Checkpoint actual:** `1.0.0-rc.2` cambia íntegramente la identidad pública a KiroMail: interfaz, documentación, paquete, Docker, base de datos nueva, credenciales locales, cookies, claves API, métricas, cabeceras, exportaciones y backups. El icono de Kiro se utiliza como marca y favicon, y la imagen social y el README público describen el producto y su estado real. Una instalación Docker nueva `kiromail` pasa suite 42/42, lint, build, bootstrap productivo y 25 controles E2E (`3c0c2226-109b-49d9-870f-49930961e387`). Se conserva la etiqueta Git `v1.0.0-rc.1` como punto recuperable anterior al cambio de nombre. Solo quedan las acciones externas de servidor, DNS, cuenta SES real y clientes de correo.
