# Serenity Mail — Especificación funcional completa

**Versión:** 1.0.0-rc.1
**Fecha:** 7 de agosto de 2026
**Estado:** release candidate preparado para instalación de producción
**Objetivo:** definir todo lo necesario para convertir Serenity Mail en una plataforma autoinstalable de newsletters y email transaccional, completa, fiable y cómoda de usar con Amazon SES.
...
**Estado global:** release candidate preparado para instalación de producción
**Checkpoint actual:** `1.0.0-rc.1` cierra la revisión final del repositorio. El arranque productivo separa migraciones, bootstrap y demo; `verify:bootstrap` demostró una base nueva con un administrador y cero contactos, listas, campañas o plantillas, y rechazó el seed sin autorización. Ajustes usa siete pestañas reales, campos condicionales, navegación móvil, preparación visible, usuarios desactivados plegados y revocación de las demás sesiones. El runbook único, secretos no destructivos, rotación de logs y readiness estricto están documentados. Suite 40/40, lint sin avisos, build, Compose producción, actualización/rollback y `verify:production` de 25 controles (`ff7a4e70-36ce-4bec-ae13-9a4fb3d6dabd`) pasan. Solo quedan las acciones externas de servidor, DNS, cuenta SES real y clientes de correo.
...
| 8. Producción | Completado localmente | Docker, healthchecks separados, workers BullMQ, RBAC, sesiones revocables, scrypt, fuerza bruta, recuperación, TOTP, cifrado, CSRF, CSP/HSTS, rate limiting Redis, request ID, logs, Prometheus, heartbeats, DLQ, filesystem/S3 y retención validados. App/worker bajan a UID 100; los volúmenes históricos se reparan de forma limitada. Backup AES-256 de 170.480 bytes y restauración real de 52 tablas superados. Un stack aislado desde cero verificó migraciones, secretos `*_FILE`, contraseña Redis obligatoria y readiness HTTPS. Carga con 100.000 contactos, accesibilidad WCAG AA y actualización/rollback están verificadas y documentadas. PostgreSQL reserva 256 MiB de memoria compartida para sostener listados paralelos del volumen de referencia. |
...
- **Siguiente acción exacta:** aprovisionar el servidor definitivo, apuntar el DNS, crear los secretos con `npm run prod:secrets`, conectar la cuenta SES y Topics SNS reales siguiendo `docs/produccion.md`, y repetir readiness y `verify:production` antes de habilitar envíos.

---

## 1. Visión del producto

Serenity Mail será una aplicación web autohospedada para gestionar suscriptores, newsletters, plantillas, campañas, emails transaccionales y resultados. Debe cubrir tanto el trabajo habitual de una plataforma de email marketing como el envío individual provocado por aplicaciones externas, manteniendo una interfaz clara, una API de primer nivel y una arquitectura operable mediante Docker.
...
Sí se proporcionará una API completa para administrar audiencias, plantillas y campañas, además de enviar emails transaccionales. También permitirá que formularios externos creen suscripciones, pero no habrá un diseñador de formularios dentro de Serenity Mail.
...
Las plantillas serán propiedad de Serenity Mail; no dependerán de las plantillas almacenadas en SES. SES actuará como transporte para que el renderizado, el versionado y la previsualización histórica tengan una única fuente de verdad.
...
Google exige a los remitentes masivos autenticación, alineación, bajas de un clic y mantener baja la tasa de spam. Serenity Mail debe mostrar estos requisitos como controles de producción, no como documentación escondida.
...
```json
{
  ...
  "from": { "name": "Serenity", "email": "news@example.com" },
  "content": {
    "html": "<html><body>...</body></html>",
...
Serenity Mail no se considerará listo para uso real hasta demostrar todos estos recorridos:
